...
module.exports = function(config) {
    ...
    return {
        ...
        callController: function ussdCallController(data) {
            ...
            var tasks = [];
            if (hooks.beforeController) {
                tasks.push(hooks.beforeController);
            }
            tasks.push(function(params) {
                return when.lift(controller).call(context, params)
                    .then(function(result) {
                        if (!_.isObject(result)) {
                            result = {};
                        }
                        result.system = data.system;
                        if (params.system.redirect) {
                            result.system.state = params.system.state;
                        }
                        return result;
                    });
            });
            if (hooks.afterController) {
                tasks.push(hooks.afterController);
            }
            return pipeline(tasks, clonedData)
                .catch(function(err) {
                    throw new Error(((err.message || '') + ' | js error thrown by controller at state ' + data.system.state));
                });
        },
