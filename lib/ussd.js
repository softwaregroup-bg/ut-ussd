    ...
    return {
        ...
        route: function(data) {
            ...
            if (data.system.expire && (new Date(data.system.expire) < new Date())) { // session expired
                ...
                var router = require(Path.join(states, data.system.currentState.split('?')[0], 'router.json'));
        ...
        callController: function(data) {
            ...
            try {
                controller = require(Path.join(states, state.split('?')[0], 'controller.js'));
            } catch (e) {
        ...
        render: function(data) {
            return loadTemplate(Path.join(states, data.system.currentState.split('?')[0], 'view.marko')).render(data)
